...
var productPolicyFiles = {
  gpt5: loadTextContent('gpt5-policy.xml')
  gpt5mini: loadTextContent('gpt5mini-policy.xml')
  unlimited: loadTextContent('unlimited-policy.xml')
}
...
resource productPolicy 'Microsoft.ApiManagement/service/products/policies@2024-06-01-preview' = [for (product, i) in apimProductsConfig: if(length(apimProductsConfig) > 0) {
  ...
  properties: {
    format: 'rawxml'
    value: replace(productPolicyFiles[product.name], '{tokens-per-minute}', '${product.tpm}')
  }
}]
    ...
    definition: {
      ...
      actions: {
        Update_APIM_Subscription_Status: {
          ...
          type: 'Http'
          inputs: {
            uri: '${environment().resourceManager}subscriptions/${subscription().subscriptionId}/resourceGroups/${resourceGroup().name}/providers/Microsoft.ApiManagement/service/${apim.name}/subscriptions/@{triggerBody()?[\'data\']?[\'alertContext\']?[\'condition\']?[\'allOf\']?[0]?[\'dimensions\']?[0]?[\'value\']}?api-version=2024-06-01-preview'
            method: 'PATCH'
            headers: {
